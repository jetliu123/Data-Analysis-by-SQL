select 
    ...
    b.ydnewcustomeraddqw as `云店新客(有下单)添加数`,
    c.dynewvisit as `云店新客添加企微访问云店数`,
    c.qwoldvisit as `企微老客添加企微访问云店数`,
    d.threemonthfirstba as `云店新客首单转化BA数`,
    d.threemonthsecondba as `云店新客二单转化BA数`,
    d.threemonththirdba as '云店新客三单转化BA数',
    e.firstnumbers as `云店新客首单转化顾客数`,
    e.firstorders as `云店新客首单订单数`,
...
select 
    ...
    row_number() over(partition by t3.ba_id,t3.user_union_id order by t3.time asc) as orderseries  -- 41007225 51029598
from 
(
select
        aa.ds as ds,
...
select 
    ...
    sum(case when aa1.ds <= t5.d then aa3.gmv end) thirdgmv      
from 
(
select 
    add_time,
